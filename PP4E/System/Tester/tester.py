"""
############################################################################## 
Тестирует сценарии Python в каталоге, передает им аргументы командной строки, 
выполняет перенаправление stdin, перехватывает stdout, stderr и код 
завершения, чтобы определить наличие ошибок и отклонений от предыдущих 
результатов выполнения. 
Запуск сценариев и управление потоками ввода-вывода производится с помощью 
переносимого модуля subprocess (как это делает функция os.popen3 в Python 2.X). 
Потоки ввода-вывода всегда интерпретируются модулем subprocess как двоичные. 
Стандартный ввод, аргументы, стандартный вывод и стандартный вывод ошибок 
отображаются в файлы, находящиеся в подкаталогах.
Этот сценарий командной строки позволяет указать имя тестируемого каталога
и флаг принудительной генерации выходного файла. Этот программный код можно 
было бы упаковать в функцию, однако то обстоятельство, что результатами 
сценария являются сообщения и выходные файлы, снижает практическую пользу 
модели вызов/ возвращаемое значение.
Дополнительные возможные расширения: можно было бы реализовать по несколько 
наборов аргументов командной строки и/или входных файлов для каждого т
естируемого сценария и запускать их по несколько раз (использовать функцию glob 
для выборки нескольких файлов “.in*” в каталоге Inputs).
Возможно, было бы проще хранить все файлы, необходимые для проведения тестов, 
...
и stdout в подкаталоге Errors, но я предпочитаю иметь ожидаемый/фактический 
вывод в подкаталоге Outputs.
Сценарий регрессивного тестирования 409
 ##############################################################################
"""
...
for testpath in testfiles:
    ...
    # получить входной файл и аргументы для тестируемого сценария
    ...
    # местоположение файлов для сохранения stdout и stderr, 
    ...
    # запустить тестируемый сценарий, перенаправив потоки ввода-вывода
